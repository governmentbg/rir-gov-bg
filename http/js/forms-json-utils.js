...
// NEW
	if($("#label_2_producer_id").length == 1)
	{
	    if($("#label_2_internal_producer_id").val().length === 0 && $("#label_2_producer_id").val().length === 0)
	    {
	    alert("Моля въведете Производител (вътрешен или външен) на Информационната система");
		bResult = false;
	    }
	}
// END NEW	
...
function ValidatePasswordField(id, sFormId)
{
	...
	if (bChanged)
	{
		if(!$('#'+id).val().match(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z!@#$&()\\-`.+]{8,}$/)){
			oFormErrors['err_'+sFormId]++;
			if (oFormErrors['input_'+sFormId] == null)
			{
				// Let's remember the first input with an error, so that we can put back the focus on it later
			...
			// Visual feedback
			$('#v_'+id).html('<img src="../images/validation_error.png"  style="vertical-align:middle"/>Паролата трябва да съдържа минимум 8 символа, главна, малка буква и число');
			return false;
		}
